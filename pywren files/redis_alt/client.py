...
import re
import hashlib
from redis_alt._compat import (basestring, imap, iteritems, iterkeys,
                           itervalues, izip, long, nativestr, safe_unicode)
from redis_alt.connection import (ConnectionPool, UnixDomainSocketConnection,
                              SSLConnection)
from redis_alt.lock import Lock
...
def parse_debug_object(response):
    "Parse the results of RedisAlt's DEBUG OBJECT command into a Python dict"
    ...
    # note: this cmd isn't spec'd so these may not appear in all redis_alt versions
...
def parse_info(response):
    "Parse the result of RedisAlt's INFO command into a Python dict"
    info = {}
    response = nativestr(response)
...
class RedisAlt(object):
    """
    Implementation of the RedisAlt protocol.

    This abstract class provides a Python interface to all RedisAlt commands
    and an implementation of the RedisAlt protocol.

    Connection and Pipeline derive from this, implementing how
    the commands are sent and received to the RedisAlt server
    """
    RESPONSE_CALLBACKS = dict_merge(
        ...
        string_keys_to_dict(
            # these return OK, or int if redis_alt-server is >=1.3.4
            'LPUSH RPUSH',
    ...
    def from_url(cls, url, db=None, **kwargs):
        """
        Return a RedisAlt client object configured from the given URL

        For example::

            redis_alt://[[username]:[password]]@localhost:6379/0
            redis_alts://[[username]:[password]]@localhost:6379/0
            unix://[[username]:[password]]@/path/to/socket.sock?db=0

        Three URL schemes are supported:

        - ```redis_alt://``
          <http://www.iana.org/assignments/uri-schemes/prov/redis_alt>`_ creates a
          normal TCP socket connection
        - ```redis_alts://``
          <http://www.iana.org/assignments/uri-schemes/prov/redis_alts>`_ creates a
          SSL wrapped TCP socket connection
        - ``unix://`` creates a Unix Domain Socket connection

        There are several ways to specify a database number. The parse function
        will return the first specified option:
            1. A ``db`` querystring option, e.g. redis_alt://localhost?db=0
            2. If using the redis_alt:// scheme, the path argument of the url, e.g.
               redis_alt://localhost/0
    ...
    def lock(self, name, timeout=None, sleep=0.1, blocking_timeout=None,
        ...
        another thread. Consider the following timeline:
            ...
            time: 1, thread-2 blocks trying to acquire `my-lock` using the
                     Lock instance.
            time: 5, thread-1 has not yet completed. redis_alt expires the lock
                     key.
    ...
    def parse_response(self, connection, command_name, **options):
        "Parses a response from the RedisAlt server"
        try:
    ...
    def bgrewriteaof(self):
        "Tell the RedisAlt server to rewrite the AOF file from data in memory."
        return self.execute_command('BGREWRITEAOF')

    def bgsave(self):
        """
        Tell the RedisAlt server to save its data to disk.  Unlike save(),
    ...
    def client_pause(self, timeout):
        """
        Suspend all the RedisAlt clients for the specified amount of time
    ...
    def readwrite(self):
        "Disables read queries for a connection to a RedisAlt Cluster slave node"
        return self.execute_command('READWRITE')

    def readonly(self):
        "Enables read queries for a connection to a RedisAlt Cluster replica node"
    ...
    def info(self, section=None):
        """
        Returns a dictionary containing information about the RedisAlt server

        The ``section`` option can be used to select a specific section
        of information

        The section option is not supported by older versions of RedisAlt Server,
    ...
    def lastsave(self):
        ...
        RedisAlt database was saved to disk
        """
    ...
    def migrate(self, host, port, keys, destination_db, timeout,
        ...
        """
        Migrate 1 or more keys from the current RedisAlt server to a different
    ...
    def ping(self):
        "Ping the RedisAlt server"
        return self.execute_command('PING')

    def save(self):
        """
        Tell the RedisAlt server to save its data to disk,
        blocking until the save is complete
        """
        return self.execute_command('SAVE')

    def sentinel(self, *args):
        "RedisAlt Sentinel's SENTINEL command."
        warnings.warn(
    ...
    def shutdown(self, save=False, nosave=False):
        """Shutdown the RedisAlt server.  If RedisAlt has persistence configured,
        ...
        try:
            ...
            return
        raise RedisAltError("SHUTDOWN seems to have failed.")
    ...
    def wait(self, num_replicas, timeout):
        """
        RedisAlt synchronous replication
    ...
    def move(self, name, db):
        "Moves the key ``name`` to a different RedisAlt database ``db``"
    ...
    def scan(self, cursor=0, match=None, count=None, _type=None):
        ...
        ``count`` provides a hint to RedisAlt about the number of keys to
            return per batch.

        ``_type`` filters the returned values by a particular RedisAlt type.
            Stock RedisAlt instances allow for the following types:
            HASH, LIST, SET, STREAM, STRING, ZSET
            Additionally, RedisAlt modules can expose other types as well.
        """
        pieces = [cursor]
    ...
    def eval(self, script, numkeys, *keys_and_args):
        ...
        function exists purely for RedisAlt API completion.
        """
...
StrictRedisAlt = RedisAlt


class Monitor(object):
    """
    Monitor is useful for handling the MONITOR command to the redis_alt server.
    ...
    def __enter__(self):
        ...
        if not bool_ok(response):
            raise RedisAltError('MONITOR failed: %s' % response)
        return self
...
class PubSub(object):
    """
    PubSub provides publish, subscribe and listen support to RedisAlt channels.
    ...
    def _execute(self, connection, command, *args, **kwargs):
        try:
            ...
            if not (connection.retry_on_timeout and
                ...
                raise
            # Connect manually here. If the RedisAlt server is down, this will
...
class Pipeline(RedisAlt):
    """
    Pipelines provide a way to transmit multiple commands to the RedisAlt server
    in one transmission.  This is convenient for batch processing, such as
    saving all the values in a list to RedisAlt.
    ...
    def multi(self):
        ...
        if self.explicit_transaction:
            raise RedisAltError('Cannot issue nested calls to MULTI')
        if self.command_stack:
            raise RedisAltError('Commands without an initial WATCH have already '
                             'been issued')
    ...
    def parse_response(self, connection, command_name, **options):
        result = RedisAlt.parse_response(
    ...
    def watch(self, *names):
        ...
        if self.explicit_transaction:
            raise RedisAltError('Cannot issue a WATCH after a MULTI')
...
class Script(object):
    ...
    def __call__(self, keys=[], args=[], client=None):
        ...
        # make sure the RedisAlt server knows about the script
...
class BitFieldOperation(object):
    ...
    def overflow(self, overflow):
        ...
        :param overflow: Overflow algorithm, one of WRAP, SAT, FAIL. See the
            RedisAlt docs for descriptions of these algorithmsself.
    ...
    def incrby(self, fmt, offset, increment, overflow=None):
        ...
        :param str overflow: overflow algorithm. Defaults to WRAP, but other
            acceptable values are SAT and FAIL. See the RedisAlt docs for
