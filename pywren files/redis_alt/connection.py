...
from redis_alt._compat import (xrange, imap, byte_to_chr, unicode, long,
...
from redis_alt.utils import HIREDIS_AVAILABLE
...
class HiredisParser(BaseParser):
    ...
    def __init__(self, socket_read_size):
        if not HIREDIS_AVAILABLE:
            raise RedisAltError("Hiredis is not installed")
        self.socket_read_size = socket_read_size
...
class Connection(object):
    "Manages TCP communication to and from a RedisAlt server"
    ...
    def connect(self):
        "Connects to the RedisAlt server if not already connected"
        if self._sock:
            return
        try:
            sock = self._connect()
    ...
    def on_connect(self):
        ...
        if self.username or self.password:
            ...
            try:
                ...
                # a username and password were specified but the RedisAlt
    ...
    def disconnect(self):
        "Disconnects from the RedisAlt server"
    ...
    def send_packed_command(self, command, check_health=True):
        "Send an already packed command to the RedisAlt server"
        if not self._sock:
            self.connect()
    ...
    def send_command(self, *args, **kwargs):
        "Pack and send a command to the RedisAlt server"
    ...
    def pack_command(self, *args):
        "Pack a series of arguments into the RedisAlt protocol"
        output = []
        # the client might have included 1 or more literal arguments in
        # the command name, e.g., 'CONFIG GET'. The RedisAlt server expects these
    ...
    def pack_commands(self, commands):
        "Pack multiple commands into the RedisAlt protocol"
        output = []
        pieces = []
        buffer_length = 0
        buffer_cutoff = self._buffer_cutoff
...
class SSLConnection(Connection):
    ...
    def __init__(self, ssl_keyfile=None, ssl_certfile=None,
        ...
        if not ssl_available:
            raise RedisAltError("Python wasn't built with SSL support")
...
class ConnectionPool(object):
    ...
    def from_url(cls, url, db=None, decode_components=False, **kwargs):
        ...
        if url.scheme == 'unix':
            ...
            raise ValueError('RedisAlt URL must specify one of the following'
    ...
    def get_connection(self, command_name, *keys, **options):
        ...
        with self._lock:
            ...
            try:
                # ensure this connection is connected to RedisAlt
                connection.connect()
...
class BlockingConnectionPool(ConnectionPool):
    ...
    Thread-safe blocking connection pool::
        ...
        >>> from redis.client import RedisAlt
        >>> client = RedisAlt(connection_pool=BlockingConnectionPool())
