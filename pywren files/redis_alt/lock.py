...
import uuid
from redis_alt.exceptions import LockError, LockNotOwnedError
from redis_alt.utils import dummy


class Lock(object):
    """
    A shared, distributed Lock. Using RedisAlt for locking allows the Lock
    ...
    LUA_RELEASE_SCRIPT = """
        local token = redis_alt.call('get', KEYS[1])
        if not token or token ~= ARGV[1] then
            return 0
        end
        redis_alt.call('del', KEYS[1])
        return 1
    """
    ...
    LUA_EXTEND_SCRIPT = """
        ...
        local expiration = redis_alt.call('pttl', KEYS[1])
        ...
        redis_alt.call('pexpire', KEYS[1], expiration + ARGV[2])
        return 1
    """
    ...
    LUA_REACQUIRE_SCRIPT = """
        ...
        redis_alt.call('pexpire', KEYS[1], ARGV[2])
        return 1
    """

    def __init__(self, redis_alt, name, timeout=None, sleep=0.1,
                 blocking=True, blocking_timeout=None, thread_local=True):
        """
        Create a new Lock instance named ``name`` using the RedisAlt client
        supplied by ``redis_alt``.
    ...
    def acquire(self, blocking=None, blocking_timeout=None, token=None):
        """
        Use RedisAlt to hold a shared, distributed lock named ``name``.
        ...
        if token is None:
            ...
            encoder = self.redis_alt.connection_pool.get_encoder()
    ...
    def do_acquire(self, token):
        if self.timeout:
            ...
            timeout = None
        if self.redis_alt.set(self.name, token, nx=True, px=timeout):
            return True
        return False
    ...
    def locked(self):
        ...
        """
        return self.redis_alt.get(self.name) is not None

    def owned(self):
        """
        Returns True if this key is locked by this lock, otherwise False.
        """
        stored_token = self.redis_alt.get(self.name)
