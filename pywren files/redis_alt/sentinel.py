...
from redis_alt.connection import ConnectionPool, Connection
from redis_alt.exceptions import (ConnectionError, ResponseError, ReadOnlyError,
                              TimeoutError)
from redis_alt._compat import iteritems, nativestr, xrange
...
class Sentinel(object):
    """
    RedisAlt Sentinel cluster client

    >>> from redis_alt.sentinel import Sentinel
    ...
    a normal RedisAlt connection can be specified here. If ``sentinel_kwargs`` is
    not specified, any socket_timeout and socket_keepalive options specified
    in ``connection_kwargs`` will be used.

    ``connection_kwargs`` are keyword arguments that will be used when
    establishing a connection to a RedisAlt server.
    """
    ...
    def __init__(self, sentinels, min_other_sentinels=0, sentinel_kwargs=None,
        ...
        self.sentinels = [RedisAlt(hostname, port, **self.sentinel_kwargs)
    ...
    def discover_master(self, service_name):
        """
        Asks sentinel servers for the RedisAlt master's address corresponding
    ...
    def master_for(self, service_name, redis_alt_class=RedisAlt,
                   connection_pool_class=SentinelConnectionPool, **kwargs):
        """
        Returns a redis_alt client instance for the ``service_name`` master.
        ...
        By default clients will be a redis_alt.RedisAlt instance. Specify a
        different class to the ``redis_alt_class`` argument if you desire
        something different.
        ...
        arguments to be used to initialize RedisAlt connections.
        """
        kwargs['is_master'] = True
        connection_kwargs = dict(self.connection_kwargs)
        connection_kwargs.update(kwargs)
        return redis_alt_class(connection_pool=connection_pool_class(
            service_name, self, **connection_kwargs))

    def slave_for(self, service_name, redis_alt_class=RedisAlt,
                  connection_pool_class=SentinelConnectionPool, **kwargs):
        """
        Returns redis_alt client instance for the ``service_name`` slave(s).
