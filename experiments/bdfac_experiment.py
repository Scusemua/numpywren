...
from numpywren.alg_wrappers import cholesky, tsqr, gemm, bdfac
...
''' NSDI bdfac effectiveness experiments '''
...
def run_experiment(problem_size, shard_size, pipeline, num_priorities, lru, eager, truncate, max_cores, start_cores, trial, launch_granularity, timeout, log_granularity, autoscale_policy, standalone, warmup, verify, matrix_exists, read_limit, write_limit, compute_threads_per_worker):
    ...
    if (not matrix_exists):
        ...
        X_sharded = BigMatrix("qr_test_{0}_{1}".format(problem_size, shard_size), shape=X.shape, shard_sizes=shard_sizes, write_header=True, autosqueeze=False, bucket="ben-pywren-632")
        ...
        X_sharded = BigMatrix("qr_test_{0}_{1}".format(problem_size, shard_size), autosqueeze=False, bucket="ben-pywren-632")
    ...
    try:
        while(program.program_status() == lp.PS.RUNNING):
            ...
            repeated_compute = parse_int(REDIS_CLIENT.get("{0}_repeated_compute".format(program.hash)))
            repeated_post_op = parse_int(REDIS_CLIENT.get("{0}_repeated_post_op".format(program.hash)))
            repeated_finish = parse_int(REDIS_CLIENT.get("{0}_repeated_finish".format(program.hash)))
            not_ready = parse_int(REDIS_CLIENT.get("{0}_not_ready".format(program.hash)))
            ...
            print("=======================================")
            print(f"Progress is {p}, Repeated Compute is {repeated_compute}, Repeated POST OP is {repeated_post_op}, Repeated Finishes is {repeated_finish}, Not ready Nodes scheduled are {not_ready}")
